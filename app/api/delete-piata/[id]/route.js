...
export async function DELETE(request, { params }) {
  try {
    const result =
      await client.sql`SELECT primary_image, sub_images FROM piata WHERE id = ${params.id};`
    ...
    await Promise.all(
      allImages.map(async image => {
        const key = image.replace(
          "https://relu-stepan.s3.eu-central-1.amazonaws.com/",
          ""
        )
        const deleteParams = {
    ...
    await client.sql`DELETE FROM piata WHERE id = ${params.id};`
